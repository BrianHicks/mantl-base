resource "google_compute_instance" "worker-nodes" {
  ...
  description = "${var.name} worker node #${format("%03d", count.index+1)}"
...
resource "null_resource" "bootstrap-worker" {
  count = "${var.worker_count}"
  ...
  connection {
    type = "ssh"
    user = "${var.ssh_user}"
    host = "${element(google_compute_instance.worker-nodes.*.network_interface.0.access_config.0.nat_ip, count.index)}"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo ${base64encode(element(google_compute_instance.worker-nodes.*.id, count.index))} | base64 -d | sudo tee /etc/mantl/id > /dev/null",
      "echo ${base64encode(element(google_compute_instance.worker-nodes.*.network_interface.0.address, count.index))} | base64 -d | sudo tee /etc/mantl/private_ip > /dev/null",
      "echo ${base64encode(element(google_compute_instance.worker-nodes.*.network_interface.0.access_config.0.nat_ip, count.index))} | base64 -d | sudo tee /etc/mantl/public_ip > /dev/null",
      ...
      "echo worker | sudo tee /etc/mantl/role > /dev/null",
    ]
  }
